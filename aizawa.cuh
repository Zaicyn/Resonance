...
#define HALFSTEP_INTERVAL    MICROTUBULE_PARALLEL  // 12
...
// Fails at compile time if constants drift out of alignment with each other.
// ============================================================================

#ifdef __cplusplus
static_assert(VIVIANI_SHADOW_STRIDE == MICROTUBULE_PROTOFILAMENTS,
    "Shadow stride must equal protofilament count");
static_assert((VIVIANI_FLAG_RING_SIZE & (VIVIANI_FLAG_RING_SIZE - 1)) == 0,
    "FLAG_RING_SIZE must be a power of 2 for modular ring arithmetic");
static_assert(INTERFERENCE_WINDOW >= INTERFERENCE_CYCLE,
    "INTERFERENCE_WINDOW must contain a full interference cycle (>=26)");
static_assert((INTERFERENCE_WINDOW & (INTERFERENCE_WINDOW - 1)) == 0,
    "INTERFERENCE_WINDOW must be a power of 2");
static_assert(SEAM_PHASE_SHIFT_BITS == 2,
    "SEAM_PHASE_SHIFT_BITS must be 2 — derived as round(64 * (2.0 - HOPF_Q))");
#endif
